...
| 2026-06-01 | Fix cross-farm weather forecast bug — Four changes: (1) `next.config.ts`: added `experimental.staleTimes.dynamic = 0` so the Next.js client-side Router Cache never reuses a stale layout RSC payload that still carries a previous farm's `farmId`; (2) `app/[farmId]/(dashboard)/layout.tsx`: added `export const dynamic = 'force-dynamic'` to also opt the layout out of the server-side Full Route Cache; (3) `app/farms/page.tsx`: changed redirect logic so multi-farm users see `FarmPicker` (card grid) rather than being silently sent to Farm 1 — only single-farm accounts still auto-redirect; (4) `CreateFarmWizard.tsx`: added `router.refresh()` after `router.push` to explicitly clear any Router Cache entries left over from the previous farm; (5) `app/actions/blocks.ts`: `createBlock`, `updateBlock`, `updateBlockBoundary` now also call `revalidatePath(/${farmId}/dashboard)` so the dashboard reflects new block GPS data immediately. |
